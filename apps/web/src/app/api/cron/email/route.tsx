...
async function send(user: User, emailType: string, subject: string, template: ReactNode) {
...
async function sendEmailChain(user: User) {
  ...
  const timeDiff = Math.floor((currentDate - createdAt) / (1000 * 60 * 60 * 24)); // in days
  ...
  if (timeDiff === 1 && !(await wasEmailSent(user.id, "Day 1"))) {
    ...
    // Send day 4 email
    await send(user, "Day 4", "Don’t Miss Out on Capturing More Leads with SmartLeadMagnet", <Day4 name={user.name} />); // Replace with actual Day 4 template
  }
}
